...
async function ensureUserInDb({ id, email, name }: { id: string; email: string; name: string | null }) {
  const existing = db.select().from(users).where(eq(users.id, id)).all();

  if (existing.length === 0) {
    db.insert(users).values({ id, email, name }).run();
  }
}
  ...
  callbacks: {
    ...
    async jwt({ token, account }) {
      ...
      try {
        ...
        token.expires_at = credentials.expiry_date ?? null;
